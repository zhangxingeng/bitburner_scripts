...
/**
 * Contracts manager (docs/design/11) — STUB. Wave 1 replaces the body.
 *
 ...
 *
 * Reference: wraps the existing /player/contract_solver.js in a periodic availability-guarded loop.
 */
export async function main(ns: NS): Promise<void> {
	...
	while (true) {
		saveSubsystem(ns, {
			...
			enabled: true,
			running: false,
			headline: 'Contracts — pending Wave 1',
			metrics: {},
			ts: Date.now(),
		});

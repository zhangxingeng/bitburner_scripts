...
/**
 * Grafting manager (docs/design/11) — STUB. Wave 1 replaces the body.
 *
 ...
 *
 * Reference: Singularity grafting (ns.grafting.*); queue by value, spend = decision.
 */
export async function main(ns: NS): Promise<void> {
	...
	while (true) {
		saveSubsystem(ns, {
			...
			enabled: true,
			running: false,
			headline: 'Grafting — pending Wave 1',
			metrics: {},
			ts: Date.now(),
		});

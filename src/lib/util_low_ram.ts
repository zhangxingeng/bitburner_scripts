 ...
 */
export async function* scanNetwork(ns: NS, fromServer: string = 'home'): AsyncGenerator<string[]> {
 ...
 */
export async function serverExists(ns: NS, server: string): Promise<boolean> {
    for await (const path of scanNetwork(ns)) {
 ...
 */
export async function getServerPath(ns: NS, toServer: string, fromServer: string = 'home'): Promise<string[]> {
    for await (const path of scanNetwork(ns, fromServer)) {
 ...
 */
export async function findAllPaths(ns: NS, fromServer: string = 'home'): Promise<Map<string, string[]>> {
 ...
 */
export async function findAllServers(ns: NS, fromServer: string = 'home'): Promise<string[]> {
    return Array.from((await findAllPaths(ns, fromServer)).keys());
}
 ...
 */
export async function getPaths(ns: NS, regex_str: string, pathCache: Map<string, string[]> | undefined = undefined): Promise<Map<string, string[]>> {
    const serverPaths = pathCache ?? await findAllPaths(ns);
 ...
 */
export async function calculateHackThreads(ns: NS, target: string, hackFraction: number = 0.5): Promise<number> {

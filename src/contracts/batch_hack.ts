import { NS } from '@ns';
import { Allocator } from '../engine/allocator';
...
const CONFIG = {
    ...
    SCRIPT_PATHS: {
        hack: '/deploy/hack.js',
        grow: '/deploy/grow.js',
        weaken1: '/deploy/weaken.js',
        weaken2: '/deploy/weaken.js',
...
async function nukeAll(ns: NS): Promise<void> {
    ...
    } catch (error) { ns.print(`WARN: nukeAll failed: ${String(error)}`); }
}
...
async function buyPortOpeners(ns: NS): Promise<void> {
    ...
    } catch (error) { ns.print(`WARN: buyPortOpeners failed: ${String(error)}`); }
}
...
async function purchaseServers(ns: NS): Promise<void> {
    ...
    } catch (error) { ns.print(`WARN: purchaseServers failed: ${String(error)}`); }
}
...
async function upgradeHomeServer(ns: NS): Promise<void> {
    ...
    } catch (error) { ns.print(`WARN: upgradeHomeServer failed: ${String(error)}`); }
}
        ...
        while (true) {
            try {
                ...
                if (currentTime - lastTargetCheck >= TARGET_CHECK_INTERVAL) {
                    ...
                    // If home RAM is violated, kill the lowest-DPS batch
                    if (ramViolated && activeBatches.size > 0) {
                        ...
                        if (targets.length > 0) {
                            ...
                            // Surgically kill only scripts targeting this specific server
                            for (const server of availableServers) {
                                const procs = ns.ps(server);
                                for (const proc of procs) {
                                    if (proc.args.length > 0 && proc.args[0] === targetToKill) {
                                        ns.kill(proc.pid);
                                    }
                                }
                            }

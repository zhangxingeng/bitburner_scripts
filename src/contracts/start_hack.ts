...
export async function main(ns: NS): Promise<void> {
    ...
    if (ns.scriptRunning('/contracts/batch_hack.js', 'home')) {
        ns.scriptKill('/contracts/batch_hack.js', 'home');
    ...
    for (const script of remoteScripts) {
        if (!ns.fileExists(`/deploy/${script}`)) {
            anyMissing = true;
            break;
        }
    }
    ...
    switch (args.mode) {
        case 'batch':
            ...
            ns.run('/contracts/batch_hack.js', 1, '--homeRam', args.homeRam, '--targets', args.targets);
            break;
